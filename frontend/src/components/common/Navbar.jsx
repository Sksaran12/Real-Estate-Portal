...
import { Building2, Heart, PlusCircle, User, LogOut, ShieldCheck, Menu, X, LogIn, UserPlus } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden md:flex items-center space-x-8 font-medium text-slate-700">
              <Link to="/properties" className="hover:text-brand-600 transition-colors font-semibold">
                Browse Properties
              </Link>
              <Link to="/properties?propertyType=rent" className="hover:text-brand-600 transition-colors font-semibold">
                Rent
              </Link>
              <Link to="/properties?propertyType=sale" className="hover:text-brand-600 transition-colors font-semibold">
                Buy
              </Link>
            </nav>
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              {/* Auth Dropdown or Login Modal Openers */}
              {user ? (
                ...
                <div className="flex items-center space-x-3">
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl font-semibold text-slate-700 hover:text-brand-600 hover:bg-slate-100 transition-colors flex items-center space-x-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl font-semibold text-white bg-brand-600 hover:bg-brand-700 transition-all shadow-md shadow-brand-500/20 active:scale-95 flex items-center space-x-1.5"
                  >
            ...
            <div className="md:hidden py-4 border-t border-slate-200 space-y-3 font-medium">
              ...
              {user ? (
                <>
                  ...
                  <button
                    onClick={openLogout}
                    className="w-full text-left px-3 py-2 rounded-lg text-red-600 hover:bg-red-50 font-semibold"
                  >
                    Sign Out
                  </button>
                </>
                ...
                <div className="pt-2 flex flex-col space-y-2">
                  <button
                    type="button"
                    onClick={() => openAuth('login')}
                    className="w-full py-2.5 rounded-xl font-semibold border border-slate-300 text-slate-700"
                  >
                    Log In
                  </button>
                  <button
                    type="button"
                    onClick={() => openAuth('register')}
                    className="w-full py-2.5 rounded-xl font-semibold text-white bg-brand-600"
                  >
                    Get Started
                  </button>
                </div>
      ...
      {/* Interactive Backdrop Blur Auth Modal */}
      <AuthModal
      ...
      {/* Interactive Backdrop Blur Logout Confirmation Modal */}
      <LogoutModal
