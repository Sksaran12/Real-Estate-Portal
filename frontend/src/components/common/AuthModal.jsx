...
import { X, Mail, Lock, User, Phone, ShieldCheck, Eye, EyeOff, Building, UserCheck, KeyRound, Smartphone, CheckCircle } from 'lucide-react';
import toast from 'react-hot-toast';

const AuthModal = ({ isOpen, onClose, initialMode = 'login' }) => {
  const { login, register, sendOtp, resetPasswordWithOtp, loginWithOtp, loading } = useAuth();
  const [mode, setMode] = useState(initialMode); // 'login', 'register', 'forgot-password', 'otp-login'
    ...
    if (mode === 'login') {
      ...
      if (!otpSent) {
        ...
        if (!otpInput || !newPasswordInput) {
          toast.error('Please enter the 6-Digit OTP code and your new password');
          return;
        }
        const success = await resetPasswordWithOtp(formData.email, otpInput, newPasswordInput);
        if (success) {
          onClose();
        }
      }
      ...
      if (!otpSent) {
        ...
        const success = await loginWithOtp(formData.email, otpInput);
        if (success) {
          onClose();
        }
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-navy-950/70 backdrop-blur-md animate-fade-in transition-all duration-300">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all duration-300 scale-100 space-y-0">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-navy-950 to-slate-900 p-6 text-white relative">
          ...
          <div className="space-y-1">
            ...
            <h2 className="text-2xl font-extrabold text-white tracking-tight">
              {mode === 'login' && 'Welcome Back to EstateHub'}
              {mode === 'register' && 'Create Your Account'}
              {mode === 'forgot-password' && 'Reset Password via OTP'}
              {mode === 'otp-login' && 'Sign In with 6-Digit OTP'}
            </h2>
            <p className="text-xs text-slate-300">
              {mode === 'login' && 'Sign in to access saved favorites and direct owner contacts'}
              {mode === 'register' && 'Join Guwahati & Assam premier real estate network'}
              {mode === 'forgot-password' && 'Enter your email to receive a 6-digit OTP code'}
              {mode === 'otp-login' && 'Instant passwordless sign in via 6-digit OTP code'}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <input
                type="email"
                required
                disabled={otpSent && (mode === 'forgot-password' || mode === 'otp-login')}
                placeholder="name@example.com"
                value={formData.email}
                onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 disabled:bg-slate-100"
              />
            </div>
          </div>
          ...
          {/* Password (Login or Register mode) */}
          {(mode === 'login' || mode === 'register') && (
            <div>
          ...
          {/* OTP Sent Alert Box */}
          {otpSent && generatedOtpCode && (mode === 'forgot-password' || mode === 'otp-login') && (
            <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 space-y-1 text-xs">
              <div className="flex items-center space-x-1.5 font-bold">
                <CheckCircle className="w-4 h-4 text-emerald-600" />
                <span>6-Digit Verification Code Generated</span>
              </div>
              <p className="text-[11px]">
                Your 6-digit OTP code is: <span className="font-extrabold text-emerald-900 tracking-widest text-sm bg-emerald-100 px-2 py-0.5 rounded">{generatedOtpCode}</span> (valid for 10 mins).
              </p>
            </div>
          )}

          {/* OTP Code Input */}
          {otpSent && (mode === 'forgot-password' || mode === 'otp-login') && (
            <div>
              <label className="text-[11px] font-extrabold uppercase text-slate-600 tracking-wider">Enter 6-Digit OTP Code</label>
              <div className="relative mt-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold tracking-widest focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                />
              </div>
            </div>
          )}

          {/* New Password Input (Forgot Password mode step 2) */}
          {otpSent && mode === 'forgot-password' && (
            <div>
              <label className="text-[11px] font-extrabold uppercase text-slate-600 tracking-wider">Enter New Password</label>
              <div className="relative mt-1">
                <Lock className="w-4 h-4 absolute left-3.5 top-3.5 text-slate-400" />
                <input
                  type={showPassword ? 'text' : 'password'}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-2xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm shadow-lg shadow-brand-500/25 transition-all active:scale-98 mt-2 flex items-center justify-center space-x-2"
          >
            {loading ? (
              ...
              <span>{otpSent ? 'Verify OTP & Change Password' : 'Send 6-Digit OTP Code'}</span>
            ) : (
              <span>{otpSent ? 'Verify OTP & Sign In' : 'Send 6-Digit Login OTP'}</span>
            )}
          </button>
